...
describe('SUBTASKS 1.1', async () => {
  async function cleanOldData() {
    // This function delete data from the database that was created from the tests (hopefully)
    ...
    await client.query(`DELETE FROM appointments WHERE customer_id=${customerId}`);
  ...
  it(':7 Your "customers" table should have "phone" column that must be unique', async () => {
    ...
    if (!uniqueColumnsRes) assert(false);

    const columns = uniqueColumnsRes.rows.filter(col => {
      return col.table_name === 'customers' && col.column_name === 'phone';
    });
  ...
  it(':9 Your "appointments" table should have a "time" column', async () => {
    const queryColumns = `SELECT table_name, column_name, column_default FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = 'public'`;
    const columnsRes = await client.query(queryColumns);
    ...
    const columns = columnsRes.rows.filter(col => {
      return col.table_name === 'appointments' && col.column_name === 'time';
    });

    assert(columns.length > 0);
  });

  it(':10 You should have at least three rows in your "services" table', async () => {
